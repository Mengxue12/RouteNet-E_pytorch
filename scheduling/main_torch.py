...
def train_one_epoch(epoch_index, tb_writer):
    ...
    for i, (x, y) in enumerate(loader_train):    # target [batch_size, len(y)]
        ...
        print('train epoch {} batch {} loss: {:.8} MAPE {:.8}'.format(epoch_index+1, i + 1, loss.item(),val_denorm_MAPE),'cpu mem', round(psutil.virtual_memory().used/1024**2), 'MB cuda mem', torch.cuda.memory_allocated(device))
        if i % 1000 == 999:
...
for epoch in range(EPOCHS):
    ...
    # train_acc = test(ds_train)
    # test_acc = test(ds_test)
    # print(f'Epoch: {epoch+1:03d}, Train Acc: {train_acc:.4f}, Test Acc: ')
    # filepath = os.path.join(ckpt_dir, f"{epoch+1:02d}-{train_acc:.2f}")
    ...
    # suffix_loss = '_loss_'+str(avg_loss)+'_vloss_'+str(avg_vloss)
